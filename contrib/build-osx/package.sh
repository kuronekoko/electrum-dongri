...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-DNGR.app"
    exit -127
fi
...
dmg dmg Electrum_uncompressed.dmg electrum-dongri-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_uncompressed.dmg

echo "Done."
md5sum electrum-dongri-$VERSION.dmg
